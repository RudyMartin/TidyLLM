#!/usr/bin/env python3
"""
TidyLLM Enterprise: Hierarchical Workflow DAG Manager
===================================================

A compliance-focused, hierarchical workflow management system that combines:
1. Hierarchical decomposition (inspired by ROS behavior trees)
2. Decision transparency with complete audit trails
3. SPARSE agreements for pre-documented decisions  
4. Dynamic flow generation for uncertain processes
5. Integration with analysis layer for complete compliance
...
class NodeType(Enum):
    ...
    ANALYSIS = "analysis"         # Document analysis node (NEW)
...
class DecisionAudit:
    ...
    node_id: str
    timestamp: datetime
    decision_type: str  # sparse_agreement, dynamic_ai, manual_override, analysis_result
    reasoning: str
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.create_audit_record(
                ...
                0.95,
                "Direct action execution completed"
            )
            ...
            self.create_audit_record(
                ...
                0.0,
                "Action execution encountered error"
            )
...
class AnalysisNode(HierarchicalNode):
    """Document analysis node - integrates with analysis layer"""
    
    def __init__(self, 
                 node_id: str, 
                 name: str,
                 analyzer: Any,  # ModelRiskAnalyzer, EvidenceValidator, etc.
                 analyzer_method: str = "assess_document_compliance",
                 **kwargs):
        super().__init__(node_id, NodeType.ANALYSIS, name, **kwargs)
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not document_content and not document_path:
            self.status = NodeStatus.FAILURE
            self.error_message = "No document content or path provided"
            
            self.create_audit_record(
                "analysis_failed",
                "Document analysis failed: no document provided",
                0.0,
                "Missing document content or path in context"
            )
        ...
        try:
            # Execute analysis
            if document_content:
                analysis_result = getattr(self.analyzer, self.analyzer_method)(document_content)
            else:
                # Read document from path
                with open(document_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                analysis_result = getattr(self.analyzer, self.analyzer_method)(content)
            
            self.status = NodeStatus.SUCCESS
            
            # Create audit record based on analysis results
            overall_score = analysis_result.get('overall_score', 0.0)
            confidence = min(overall_score + 0.1, 1.0)  # Slight boost for having results
            
            self.create_audit_record(
                "analysis_completed",
                f"Document analysis completed with score {overall_score:.2f}",
                confidence,
                f"Analysis method: {self.analyzer_method}, Analyzer: {type(self.analyzer).__name__}"
            )
            ...
            self.create_audit_record(
                ...
                0.0,
                f"Analysis method: {self.analyzer_method}, Error: {str(e)}"
            )
...
class SparseDecisionNode(HierarchicalNode):
    ...
    def __init__(self, 
                 node_id: str, 
                 name: str, 
                 sparse_agreement: Any,  # SparseAgreement object
                 **kwargs):
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (hasattr(self.sparse_agreement, 'expiry_date') and 
            self.sparse_agreement.expiry_date and 
        ...
        # For demo purposes, assume agreement is valid and execute
        self.status = NodeStatus.SUCCESS
        self.end_time = datetime.now()
        ...
        return {
            ...
            'actions_executed': [action.name for action in getattr(self.sparse_agreement, 'approved_actions', [])],
            'compliance_framework': [f.value for f in getattr(self.sparse_agreement, 'compliance_frameworks', [])],
            'execution_time': self._get_execution_time()
        }

class DynamicFlowNode(HierarchicalNode):
    """AI-generated flow for uncertain processes"""
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.create_audit_record(
                ...
                0.85,
                "AI-generated workflow executed successfully"
            )
