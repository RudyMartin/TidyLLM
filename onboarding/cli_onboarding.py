...
Usage:
    python cli_onboarding.py                    # Interactive wizard
    python cli_onboarding.py --config-only      # Generate config from template
    python cli_onboarding.py --validate         # Validate existing setup
"""
...
╚══════════════════════════════════════════════════════════════════╝
    ...
    def validate_aws_connectivity(self, region: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use the session validator to test connectivity
            results = test_full_aws_stack(region=region)
            
            if results.get('overall_success', False):
                rprint("✅ AWS connectivity test passed!")
            else:
                rprint("❌ AWS connectivity test failed")
            ...
            rprint("❌ session_validator not available - basic validation only")
            return {"valid": False, "error": "session_validator not installed"}
...
- Test connectivity: `python cli_onboarding.py --validate`
...
def main():
    ...
    if args.validate:
        ...
        if all(env_check.values()) and aws_check.get("overall_success", False):
