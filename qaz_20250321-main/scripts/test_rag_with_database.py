...
# -*- coding: utf-8 -*-
"""
Comprehensive RAG Test with Database Integration

This script tests the complete RAG pipeline:
1. PDF processing and chunking
2. Database storage with embeddings
...
def store_chunks_in_database(chunks: List[Dict]) -> bool:
    ...
    try:
        ...
        with conn.cursor() as cursor:
            # Note: document_chunks table already exists with different structure
...
def answer_question_with_rag(question: str, top_k: int = 3) -> Dict:
    ...
    try:
        ...
        # Simple keyword-based retrieval (in production, you'd use vector similarity)
        ...
        # Simple answer generation (in production, you'd use an LLM)
        if relevant_chunks:
            # Combine relevant text for answer
...
def test_complete_rag_pipeline():
    ...
    try:
        ...
        test_questions = [
            "What are the key best practices for model validation?",
            "What is the validation scope?",
            "How should validation steps be applied?",
            "What are the consequences of inaccurate models?"
        ]
...
def main():
    ...
    if success:
        ...
        print("1. ✅ PDF processed and chunked using pymupdf")
        print("2. ✅ Chunks stored in Aurora PostgreSQL database")
        print("3. ✅ Database tables created (document_chunks, chunk_embeddings)")
        print("4. ✅ RAG question answering tested")
        print("5. ✅ Relevant chunks retrieved from database")
