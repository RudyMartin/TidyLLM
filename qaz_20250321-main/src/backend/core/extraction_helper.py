...
import json
import boto3
# NLTK removed - using simple sentence splitting instead
import unicodedata
import logging
...
try:
    ...
    try:
        ...
        PYPDF_AVAILABLE = True
        logging.warning("Using PyPDF2 (deprecated). Consider upgrading to pypdf.")
    except ImportError:
...
def extract_text_to_json():
    ...
    try:
        ...
        logging.error(f"❌ Error in extract_text_to_json: {e}")
...
class ExtractionHelper:
    """
    Wrapper class for extraction helper functions.
    Provides a unified interface for text extraction, cleaning, and processing.
    """
    
    def __init__(self):
        """Initialize the ExtractionHelper."""
        pass
    ...
    @staticmethod
    def clean_text_backup(text: str) -> str:
        """Clean extracted text using the backup cleaning method."""
        return clean_text_backup(text)
    ...
    def validate_page_continuity(text_current: str, text_next: str) -> bool:
        ...
        return validate_page_continuity(text_current, text_next)
    
    @staticmethod
    def extract_text_from_pdf_page_s3(page_key: str, next_page_key: str = None, bucket: str = None) -> Dict:
        """Extract text from a PDF page stored in S3."""
        return extract_text_from_pdf_page_s3(page_key, next_page_key, bucket)
    
    @staticmethod
    def chunk_text_into_segments(pdf_page_data: Dict, max_words: int = None) -> List[Dict]:
        """Chunk text into segments."""
        return chunk_text_into_segments(pdf_page_data, max_words)
    
    @staticmethod
    def list_objects_s3(bucket: str, prefix: str) -> List[str]:
        """List objects in S3 bucket."""
        return list_objects_s3(bucket, prefix)
    
    @staticmethod
    def check_s3_object_tag(bucket: str, key: str, tag_key: str) -> bool:
        """Check if an S3 object has a specific tag."""
        return check_s3_object_tag(bucket, key, tag_key)
