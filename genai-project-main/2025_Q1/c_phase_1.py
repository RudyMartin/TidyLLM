...
def save_text_chunk_as_json_s3(config: Dict, original_key: str, chunks: List[str]) -> List[str]:
    """Creates JSON files for each text chunk and stores them in S3, tracking text extraction date."""
    output_keys = []
    try:
        document_name = os.path.basename(original_key)
        page_number = original_key.split("_")[-1].split(".")[0]
        text_date = datetime.now().isoformat()  # Capture extraction timestamp
        ...
        for i, chunk in enumerate(chunks):
            chunk_data = {
                ...
                "chunk_number": i + 1,
                "text_date": text_date  # Store extraction date
            }
