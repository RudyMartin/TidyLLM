    ...
    try:
        ...
        for i, paper in enumerate(papers):
            ...
            try:
                ...
                # Calculate semantic relevance for tracking
                semantic_relevance = framework._calculate_semantic_relevance(
...
def search_and_track(query, source="ArXiv", max_results=10):
    ...
    try:
        ...
        if success:
            return True, f"Successfully tracked {len(papers)} papers", len(papers)
        else:
            return False, "Failed to track search", len(papers)
